...
`test_shadow_portfolio.py` 涵蓋）：
  1. `FORWARD_V1_START_DATE` 之前只跑 v1_frozen，之後才加入 FORWARD_V1_202609
  2. 某個 strategy_version 執行失敗不影響其他 strategy_version 照跑
"""
...
def test_strategy_versions_for_date_includes_forward_v1_from_start_date_onward():
    versions_on_start = runner._strategy_versions_for_date(runner.FORWARD_V1_START_DATE)
    assert versions_on_start == [sp.STRATEGY_VERSION, sp.STRATEGY_VERSION_FORWARD_V1]

    after = runner.FORWARD_V1_START_DATE + timedelta(days=30)
    versions_after = runner._strategy_versions_for_date(after)
    assert versions_after == [sp.STRATEGY_VERSION, sp.STRATEGY_VERSION_FORWARD_V1]
