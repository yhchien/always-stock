...
| Continuation（強勢延續）核心桶 | 300,000 元 | 強勢延續型股票，最多 3 個單位 |
| Pullback（拉回回穩）桶 | 100,000 元 | 拉回後重新回穩型股票，最多 1 個單位 |
| Opportunity（機會）桶 | 300,000 元 | Continuation 核心桶滿載後的額外機會，最多 3 個單位 |
| Continuation Starter（延續首次進場） | 100,000 元 | 新的延續型部位首次買進金額 |
| Confirmation（確認）加碼 | 0 元 | 目前只確認狀態，不另外投入第二筆資金 |
| 循環長度 | 35 個交易日 | 到期全部結算，下一循環本金重設 |

Opportunity（機會）是資金桶，不是獨立選股邏輯。符合哪一種進場 profile（進場型態），先決定它屬於哪個
...
### C. 桶子滿載時的 Opportunity（機會）/ Rotation（輪動換股）
...
因此新加入的 Early High Momentum（早期高動能，動能至少 85）主要使用核心桶；核心桶滿時，可能因
