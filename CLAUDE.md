...
## 重跑 2026-09-08 完整選股 flow 驗證修法：確認生效 + 抓到一個新的細產業用詞不精確案例（2026-09-09 第三輪）

### 背景
上一輪（Tier 1 + Tier 2 修法）完成後，使用者要求實際重跑 9/4 跟 9/8 兩天的選股 flow
驗證效果。討論範圍後改成只跑 **9/8 一天的完整 pipeline**（含魚尾追蹤 `signal_watch_
hits` 與 P4 觀察，不是只重產 snapshot）——直接用正式排程入口
`python run_daily_signals.py 2026-09-08`，跟平常 cron 觸發是同一支程式，
`signal_snapshots` 本來就是 UPSERT by date 會直接覆蓋。

### 驗證結果
- **玉晶光（3406）**：分類正確顯示「電腦及週邊設備/光學鏡片、鏡頭」，**不再被
  BUSINESS_MISMATCH/THEME_MISMATCH 否決**——這次改成被 `MOMENTUM_NOT_FRESH`
  （動能新鮮度過期，deterministic 判斷）刷掉，是合理的下架理由，證實 Tier 1 校正
  確實生效
- **AMAX-KY（6933）**：分類正確顯示「伺服器液冷散熱與整機櫃」後，**直接進入正式
  WATCH 名單**（FOLLOWER）——證實從 `LOW confidence/review_required=True` 補上
  明確分類後，真的能讓候選被 LLM 正常評估、甚至入選
- **鼎元（2426）**：意外抓到一個新問題——2026-09-09 第一輪幫它補的 sub_sector 猜成
  「化合物半導體元件/雷射二極體晶片」，但 LLM 今天即時查證後說真實產品是「可見光
  LED、紅外線LED、受光元件與光二極體」，因為文字對不上（雷射二極體 vs LED/受光元件）
  又被判 `THEME_MISMATCH` 否決。這**不是** Tier 1/Tier 2 兩個修法失效——是我當初補
  override 時自己猜的 sub_sector 描述用詞不夠精確

### 修法
`stock_overrides.py`：2426 的 sub_sector 從「化合物半導體元件/雷射二極體晶片」改成
「LED磊晶/晶粒/受光元件」（直接採用 LLM 今天查證過的真實產品用詞）；順手把 2455 的
sub_sector 也拿掉同樣未經即時查證的「雷射二極體」猜測用詞，改成較保守的「LED磊晶/
晶粒」，降低未來被同一類問題命中的風險。重跑 `run_classification_backfill.py` 套用。

### 教訓
- **重跑驗證是必要的，光靠邏輯推導無法保證分類文字本身精確**：這次如果沒有實際重跑
  9/8，不會發現「我自己補的 override 文字用詞不夠精確」這個新問題——Tier 1/Tier 2
  兩個修法都是正確、有效的架構修正，但 override 表裡**個別股票的文字描述本身還是
  可能出錯**，這是校正表維護的持續性工作，不是修完架構就一勞永逸
- **sub_sector 的猜測應該盡量貼近可查證的真實產品用詞，不要用聽起來更專業/更精確的
  詞彙過度延伸**（例如把「LED」猜成「雷射二極體」，雖然兩者都屬於光電/半導體發光元件
  的相近技術領域，但業務上是不同產品，文字精確度差一點就可能被 LLM 判不符）
- **在正式 production DB 上重跑歷史日期的完整 pipeline 前，務必先確認範圍**（本輪
  一開始問過使用者要不要限縮成只重產 snapshot，使用者確認要跑含追蹤/P4 觀察的完整
  版本）——這類操作會回頭改寫已經被後續日期依賴的追蹤狀態，屬於有風險、需要事先溝通
  範圍的操作，不能因為使用者說「覆蓋掉沒關係」就自動假設涵蓋所有下游表

## 全面盤點 8/1~9/8 BUSINESS_MISMATCH/THEME_MISMATCH 否決名單：發現比「校正表漏檔」更大的架構問題（2026-09-09 第二輪）

### 背景
使用者接續玉晶光那次修法，要求全面盤點 2026-08-01~09-08 所有進候選池且動能夠強、但被
LLM 用 `BUSINESS_MISMATCH`/`THEME_MISMATCH` 否決的股票，判斷「是不是真不對還是需要做
產業校正」，一次做好。查 `signal_snapshots.removed` 撈出 **175 檔獨立股票**（總計數百次
否決紀錄），逐一比對 LLM 研究時查到的「真實業務」文字描述 vs 我們餵給它的分類標籤，
分出 4 種完全不同的情況：

### Tier 1｜真正的資料標籤錯誤（跟南亞/玉晶光同一類 bug）
LLM 查到的真實業務跟分類標籤幾乎零重疊，確認是 FinMind 原始誤置分類、且從未被
`stock_overrides.py` 涵蓋：
- **1229 聯華**：標「雲端運算/系統整合」，實際是食品業（聯華實業，麵粉/飼料）
- **2426 鼎元**：標「電腦及週邊設備/精簡型電腦」，實際是化合物半導體元件/雷射二極體晶片廠
- **2455 全新**：標「半導體/晶圓製造」，實際是 LED 磊晶/化合物半導體元件廠（全新光電）
- **3504 揚明光**：標「電腦及週邊設備/金屬、塑膠模具」，實際是光學鏡頭/投影機鏡頭
- **4976 佳凌**：標「電腦及週邊設備/金屬、塑膠模具」，實際是手機相機鏡頭/光學元件
- **6933 AMAX-KY**：本來就標記 `confidence=LOW`/`review_required=True` 待複核，
  依 LLM 研究結果補上「伺服器液冷散熱與整機櫃」（`COMPUTER_PERIPHERALS`）

修法：`stock_overrides.py` 補 6 筆 override（2426/2455 依大立光的判斷方式，同屬光電元件
廠應與晶電(2448)一樣分在 `OPTOELECTRONICS`；3504/4976 依同業判斷應與大立光(3008)/
玉晶光(3406) 同分在 `COMPUTER_PERIPHERALS`/「光學鏡片、鏡頭」），跑
`run_classification_backfill.py` 套用。

### Tier 2｜比 Tier 1 嚴重得多的系統性問題：sub_industry 從未進題材驗證清單
**最關鍵的發現**：連**大立光（3008）自己的分類都完全正確**
（`COMPUTER_PERIPHERALS`/「光學鏡片、鏡頭」），2026 年 8 月仍被同樣理由否決 **7 次**——
證實「校正表補對了」不保證「LLM 就不會再誤殺」。追到 `app/signals/prompt_family.py::
research_input()` 才發現根因：組給 research 階段驗證的 `theme_candidates` 清單，
只有 `theme_cluster`（動態敘事標籤）與 `industry`（`primary_sector` 粗分類中文名，如
「電腦及週邊設備」）兩項，**`sub_industry`（細產業，如「光學鏡片、鏡頭」）從未被放進去**
——即使 `sub_industry` 已經精準對應公司實際業務，LLM 驗證時也只能拿粗略的大分類文字去
比對「光學鏡頭」，字面對不上就判 `MISMATCH`。這個問題的影響範圍**遠大於 175 檔清單本身**
：任何細產業比粗分類精確、且業務名詞用字不同的候選（本次清單裡至少 100+ 檔屬於這種
「分類完全正確、純粹字面比對太嚴」情況，例如 8039台虹/6213聯茂/2454聯發科/2408南亞科/
2481強茂等）都可能中招，且是天天在跑的正式 pipeline 共用邏輯，不是單一股票的資料問題。

修法（[app/signals/prompt_family.py](backend/app/signals/prompt_family.py)
`research_input()`）：`theme_candidates` 未顯式提供時，改用
`[theme_cluster, sub_industry, industry]`（`sub_industry` 排在 `industry` 之前，
因為它是更精確的候選）+ `dict.fromkeys()` 去重；同步在
[candidate-research-v7.md](backend/app/prompts/candidate-research-v7.md) 加一段
明確說明：「`theme_candidates` 只要對應到其中**任一項**即可判 VERIFIED，不需要與最
寬泛的那一項逐字相符；只有清單內每一項都明顯衝突才可判 MISMATCH」——因為 prompt 原本
完全沒提過 `theme_candidates` 這個欄位該怎麼解讀，只改資料結構、沒改 prompt 說明的話
LLM 可能還是照舊只認第一項/最寬泛那項。

### Tier 3｜LLM 研究階段自己查錯公司（幻覺，不是我們的分類問題）
驗證 DB 現況後排除：底下這些股票的 `security_classification` 本身完全正確，是 LLM
當天研究時把公司搞混：
- **2006 東和鋼鐵**：LLM 誤查成「東和紡織」（實際 2006 就是東和鋼鐵，鋼胚業務）
- **7711 永擎**：LLM 誤查成「信驊科技」（實際 7711 就是永擎，伺服器業務）
- **2539 櫻花建**：LLM 誤查成「廚具品牌」（實際是建設開發公司，可能與同名「櫻花」
  廚具品牌搞混）

這類不是分類校正能解的問題，屬於 LLM web search 查證階段的既有風險，本輪未處理
（無法用資料層修法解決一次性查錯，且非系統性重現）。

### Tier 4｜查了發現其實已經修過，不需要動作
原本清單裡有 9 檔標「其他/其他」太籠統的股票（1608華榮/1736喬山/4536拓凱/2404漢唐/
2540愛山林/8422可寧衛/2359所羅門/6830汎銓/8464億豐），直接查 DB **現況**才發現
`security_classification` 已經是具體分類（例如 1736喬山已是 `SPORTING_GOODS`/
「健身器材」、2404漢唐已是 `FACTORY_ENGINEERING`/「廠務工程」）——`removed` 快照裡
看到的「其他/其他」是**歷史資料**（這幾檔是在校正生效**之前**被否決的舊紀錄），現在
再跑就會拿到正確標籤，不需要額外修改。

### 教訓
- **「校正表補對了」不等於「LLM 以後不會再用同一個理由否決」**——這是本輪最重要的
  發現：3008 是校正表裡最早、最明確驗證正確的案例之一，卻因為下游 `research_input()`
  完全沒把 `sub_industry` 送進驗證清單，照樣被連續誤殺。修「資料是否正確」跟修「資料
  有沒有被正確使用」是兩件獨立的事，跟 [[project_nan_ya_industry_label_llm_veto_bug]]
  記錄過的教訓同一個模式，只是這次錯在更下游的一個環節
- **查「vetoed 名單裡的股票分類還是不是問題」，一定要查 DB 現況、不能只看歷史快照**：
  Tier 4 那 9 檔如果沒有重新查一次 DB 現況，會誤判成「還需要修」，浪費工夫改一個
  早就改好的東西
- **大量否決紀錄的分析，LLM 自己寫的「查到的真實業務」文字本身就是很好的 ground
  truth**：不需要逐檔靠人工記憶判斷這 175 檔各自的真實產業，直接讀 `short_reason`
  裡 LLM 已經查證過的具體業務描述，比對是否跟分類標籤矛盾，效率遠高於憑印象猜
- **系統性 code/prompt 架構問題 vs 個別資料錯誤，要分開處理、分開溝通**：本輪如果
  只顧著改 Tier 1 那 6 檔資料，會錯過影響範圍大得多的 Tier 2 架構問題；用
  `AskUserQuestion` 把兩種不同風險等級、不同修法範疇的問題分開呈現，讓使用者對
  「這次改動會動到共用邏輯、影響所有未來候選」有明確認知後再動手
...
- 對 production DB 重跑 `backfill_shadow_portfolio_replay.py --execute` 兩次：
  第一次範圍依使用者要求擴大到 **2026-08-01~09-07**（26 個交易日）：+3.26% 累積報酬、
  24 筆交易、勝率 41.7%；第二次改回**原始沙盒窗口 2026-08-07~09-04**（21 個交易日，
  apples-to-apples 對照）：**+5.51% 累積報酬、23 筆交易、勝率 43.5%**（`/api/signals/
  shadow-portfolio` 正式 API 已回傳這個結果，`as_of_trade_date=2026-09-04`）
- **誠實揭露：修復後仍未逐筆重現沙盒的 +12.05%／28 筆交易**。同一段 21 個交易日窗口，
  production port 算出 +5.51%／23 筆，比沙盒的 +12.05%／28 筆低——比對逐筆交易清單，
  兩邊有相當比例的股票完全不重疊（沙盒有但 production 沒有：6177／7711／2014／3443／
  6472／3135／2882／8150／2395 等；production 有但沙盒沒有：4967／2030／6919／3006／
  1808／2302／6933／2465／2421 等）。這代表 2383／6491／6213 這三個已經精確定位並修好
  的案例，**不是這次落差的全部成因**——很可能還有其他尚未定位的邊界情況，讓 production
  port 篩出的候選跟沙盒 CSV 匯出當下的候選有系統性差異（例如魚尾 cohort 邊界的其他
  revival/重置規則、或候選池排序在容量受限時的細微差異）。
  已修好的三個 bug（first_seen_date 來源、p4_decision 對應、is_official_exit_signal_
  day 定義）確定解決了「production 顯示負報酬」這個最嚴重的症狀（-9.09% → +5.51%，
  同一段窗口），但**尚未達到跟沙盒逐筆完全一致的程度**，下一輪若要繼續深挖，應該從
  「沙盒有、production 沒有」的股票（如 6177／7711）反查其魚尾週期在 production DB
  裡實際的 first_seen_date/day_index/hit_count_so_far，跟沙盒 CSV 對應列逐欄位比對，
  找出還有哪個欄位算法不一致。
