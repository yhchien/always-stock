...
| `backend/app/signals/candidate_pool.py`, `pipeline.py` | momentum ordering | Momentum score orders raw union and LLM processing. P1 subsequently removed the 120/50 total truncations. No post-decision `momentum_rank > N => REMOVE` or final rank cutoff exists. | No P0 final-rank removal found. | `FOUND_AND_FIXED_BY_P1` for the former capacity limits; `NOT_FOUND` for a final Momentum Rank cutoff. Frozen P3D policy permits `RANK` only. |
...
| `backend/app/signals/candidate_pool.py` | former `POOL_SOFT_TRIGGER=150`, `POOL_HARD_LIMIT=120` | P1 removed raw-union truncation; ordering remains. | Explicit P1 item. | `FOUND_AND_FIXED_BY_P1`. |
| `backend/app/signals/pipeline.py` | former `LLM_INPUT_HARD_LIMIT=50` | P1 removed LLM-input truncation; all eligible candidates are batched. | Explicit P1 item. | `FOUND_AND_FIXED_BY_P1`. |
...
P0 itself did not modify raw union 120 or LLM input 50; P1 subsequently removed both.
P0/P1 do not modify B/C/D channel caps,
