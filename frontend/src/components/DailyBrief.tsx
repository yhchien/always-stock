...
export default function DailyBrief({ date }: Props) {
  ...
  return (
    <div className="rounded-xl border border-slate-600 bg-slate-800/40 overflow-hidden">
      ...
      {content && !expanded && (
        <div
          className="px-4 py-2 cursor-pointer hover:bg-slate-600/30 transition-colors"
          onClick={() => setExpanded(true)}
        >
          <p className="text-xs text-slate-400 truncate">{content.slice(0, 80)}...</p>
        </div>
      )}

      {!loading && !error && !content && (
        <div className="px-4 py-3">
          <p className="text-xs text-slate-500">點擊「產生 AI 盤前摘要」以獲得今日市場觀察。</p>
        </div>
      )}
    </div>
  )
